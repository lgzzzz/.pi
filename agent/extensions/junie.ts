...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet: "Call junie_ai for writing documentation, or creating execution plans.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      description: Type.String({
        description: "Description of what junie_ai should do. Be specific about the task.",
      }),
    }),
    ...
    renderResult(result, options, theme, _context) {
      ...
      if (options.expanded) {
        ...
        container.addChild(
          new Text(`\n(${keyHint("app.tools.expand", "to collapse")})`, 0, 0)
        );
        return container;
      }
      ...
      let text = previewLines.map((l) => theme.fg("toolOutput", l)).join("\n");
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (!description || description.trim() === "") {
        return {
          content: [{ type: "text", text: "Error: Description cannot be empty" }],
      ...
      try {
        ...
        if (result.killed) {
          return {
            content: [{ type: "text", text: "Error: Command was killed (timeout or cancellation)" }],
        ...
        return {
          content: [{ type: "text", text: `Error executing Junie: ${message}` }],
