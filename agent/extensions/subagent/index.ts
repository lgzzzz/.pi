...
import { MAX_PARALLEL_TASKS, MAX_CONCURRENCY, COLLAPSED_ITEM_COUNT } from "../utils/constants.js";
        ...
        if (event.type === "message_end" && event.message) {
          ...
          if (msg.role === "assistant") {
            ...
            if (usage) {
              ...
              currentResult.usage.cost = (currentResult.usage.cost ?? 0) + (usage.cost?.total || 0);
    ...
    renderCall(args, theme, context) {
      ...
      if (args.tasks && args.tasks.length > 0) {
        let text =
          ...
          theme.fg("muted", ` [${scope}]`);
        const maxTasks = expanded ? args.tasks.length : Math.min(args.tasks.length, 3);
        for (let i = 0; i < maxTasks; i++) {
          const t = args.tasks[i];
          const displayTask = expanded ? t.task : (t.task.length > 40 ? `${t.task.slice(0, 40)}...` : t.task);
      ...
      const preview = expanded ? fullTask : (fullTask.length > 60 ? `${fullTask.slice(0, 60)}...` : fullTask);
      let text =
        theme.fg("toolTitle", theme.bold("subagent ")) +
    ...
    renderResult(result, { expanded }, theme, _context) {
      ...
      if (details.mode === "chain") {
        ...
        if (expanded) {
          ...
          const usageStr = formatUsageStats(aggregateUsage(details.results.map((r) => r.usage)));
          if (usageStr) {
            container.addChild(new Spacer(1));
