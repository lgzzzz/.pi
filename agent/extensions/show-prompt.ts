...
import { Container, Markdown, matchesKey, Key, Text } from "@earendil-works/pi-tui";
    ...
    handler: async (_args, ctx) => {
      ...
      await ctx.ui.custom((tui, theme, _keybindings, done) => {
        ...
        container.addChild(new Text(
          theme.fg("accent", theme.bold(" System Prompt Viewer ")),
          1,
          0
        ));
        container.addChild(new Text(
          theme.fg("muted", `Length: ${systemPrompt.length} characters`),
          1,
          0
        ));
        ...
        if (systemPrompt.trim()) {
          container.addChild(new Markdown(systemPrompt, 1, 1, mdTheme));
        } else {
          container.addChild(new Text(
